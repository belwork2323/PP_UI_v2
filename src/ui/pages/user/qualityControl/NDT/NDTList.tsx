...
const NDTList = ({ hookState, rowsPerPageOptions }: any) => {
  ...
  const statusConfig = useMemo(
    () => Object.fromEntries(
      Object.entries(NDT_STATUS_CONFIG).map(([status, cfg]) => [status, { ...cfg, ...theme.batchList.statusConfig[status] }])
    ),
    [theme]
  );
  ...
  const columns = useMemo(
    () => [
      ...
      { key: "ndtStatus", label: S.NDT.COL_STATUS, align: "center", render: (v: string, row: any) => <UserWorkflowStatusCell status={v} statusConfig={statusConfig} rejectedStatus={OPERATION_STATUS.REJECTED} rejectionReason={row.rejectionReason} theme={theme} /> },
    ],
    [statusConfig, theme]
  );
  ...
  <UserBatchList
    ...
    highlightRow={(row: any) => row.ndtStatus === OPERATION_STATUS.REJECTED}
