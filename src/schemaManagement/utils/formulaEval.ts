...
/** Evaluates simple arithmetic formulas using row field keys (e.g. `a - b`). */
...
export const applyFormulaColumns = (
  row: Record<string, unknown>,
  columns: { key: string; formula?: { expression?: string } }[]
): Record<string, unknown> => {
  const next = { ...row };
  columns.forEach((col) => {
    if (!col.formula?.expression) return;
    next[col.key] = evaluateRowFormula(col.formula.expression, next);
  });
  return next;
};
